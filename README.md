...
| Synchronization      | Description                                                            | Storage | Example Machine                            | Nethermind Sync                                        | Nethermind Pruning                                           | Geth                                    |
| ------------------------- | ---------------------------------------------------------------------- | ------------------- | ------------------------------------------ | ------------------------------------------------------ | ------------------------------------------------------------ | --------------------------------------- |
| Snap Sync <br>*(recommended)* | Downloads a recent snapshot and then catches up with the latest blocks | Min. 128 GB              | Raspberry Pi (3-4-5)                       | `--Sync.SnapSync=true` <br> or <br> `--Sync.Mode=Snap` | `--Pruning.Mode=Hybrid`                                      | `geth --syncmode snap`                  |
| Full Sync                 | Downloads and validates all blocks and transactions                    | Min. 1 TB        | Desktop computer or laptop newer than 2021 | `--Sync.Mode=Full`                                     | `--Pruning.Mode=Memory` <br> or <br> `--Pruning.Mode=Hybrid` | `geth --syncmode full`                  |
| Archive Node              | Stores all historical blockchain states                                | Min. 1 TB        | Desktop computer or laptop newer than 2021 | `--Sync.Mode=Full`                                     | `--Pruning.Mode=None`                                        | `geth --syncmode full --gcmode archive` |
